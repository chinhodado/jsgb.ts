...
function gb_CPU_ADD16(n1, n2) {
    FN = 0;
    FH = ((n1 & 0xFFF) + (n2 & 0xFFF)) > 0xFFF ? 1 : 0; // TODO test bit 11. Not sure on this
    n1 += n2;
    FC = n1 > 0xFFFF ? 1 : 0;
    n1 &= 0xFFFF;
    gbCPUTicks = 8;
...
OP[0x07] = function () { FC = (RA >> 7) & 1; RA = ((RA << 1) & 0xFF) | FC; FN = FH = 0; FZ = RA == 0 ? 1 : 0; gbCPUTicks = 4; }; // RLCA
...
OP[0x0F] = function () { FC = RA & 1; RA = (RA >> 1) | (FC << 7); FN = 0; FH = 0; FZ = RA == 0 ? 1 : 0; gbCPUTicks = 4; }; // RRCA
...
OP[0x1F] = function () { T1 = FC; FC = RA & 1; RA = (RA >> 1) | (T1 << 7); FN = 0; FH = 0; FZ = RA == 0 ? 1 : 0; gbCPUTicks = 4; }; // RRA
...
OP[0xF8] = function () { var n = MEMR(PC++); HL = SP + sb(n); FZ = 0; FN = 0; FH = (((SP & 0x0F) + (n & 0x0F)) > 0x0F) ? 1 : 0; FC = (((SP & 0xFF) + (n & 0xFF)) > 0xFF) ? 1 : 0; gbCPUTicks = 12; }; // LD HL,SP+u8;
...
OPCB[0x28] = function () { FC = RB & 1; RB = (RB >> 1) | (RB & 0x80); FN = 0; FH = 0; FZ = RB == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRA n
OPCB[0x29] = function () { FC = RC & 1; RC = (RC >> 1) | (RC & 0x80); FN = 0; FH = 0; FZ = RC == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRA n
OPCB[0x2A] = function () { FC = RD & 1; RD = (RD >> 1) | (RD & 0x80); FN = 0; FH = 0; FZ = RD == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRA n
OPCB[0x2B] = function () { FC = RE & 1; RE = (RE >> 1) | (RE & 0x80); FN = 0; FH = 0; FZ = RE == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRA n
OPCB[0x2C] = function () { var H = HL >> 8; FC = H & 1; H = (H >> 1) | (H & 0x80); FN = 0; FH = 0; FZ = H == 0 ? 1 : 0; HL = (H << 8) | (HL & 0x00FF); gbCPUTicks = 8; }; // SRA n
OPCB[0x2D] = function () { var L = HL & 0xFF; FC = L & 1; L = (L >> 1) | (L & 0x80); FN = 0; FH = 0; FZ = L == 0 ? 1 : 0; HL = (HL & 0xFF00) | L; gbCPUTicks = 8; }; // SRA n
OPCB[0x2E] = function () { var M = MEMR(HL); FC = M & 1; M = (M >> 1) | (M & 0x80); FN = 0; FH = 0; FZ = M == 0 ? 1 : 0; MEMW(HL, M); gbCPUTicks = 16; }; // SRA n
OPCB[0x2F] = function () { FC = RA & 1; RA = (RA >> 1) | (RA & 0x80); FN = 0; FH = 0; FZ = RA == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRA n
...
OPCB[0x38] = function () { FC = RB & 1; RB = RB >> 1; FN = 0; FH = 0; FZ = RB == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRL n
OPCB[0x39] = function () { FC = RC & 1; RC = RC >> 1; FN = 0; FH = 0; FZ = RC == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRL n
OPCB[0x3A] = function () { FC = RD & 1; RD = RD >> 1; FN = 0; FH = 0; FZ = RD == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRL n
OPCB[0x3B] = function () { FC = RE & 1; RE = RE >> 1; FN = 0; FH = 0; FZ = RE == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRL n
OPCB[0x3C] = function () { var H = HL >> 8; FC = H & 1; H = H >> 1; FN = 0; FH = 0; FZ = H == 0 ? 1 : 0; HL = (H << 8) | (HL & 0x00FF); gbCPUTicks = 8; }; // SRL n
OPCB[0x3D] = function () { var L = HL & 0xFF; FC = L & 1; L = L >> 1; FN = 0; FH = 0; FZ = L == 0 ? 1 : 0; HL = (HL & 0xFF00) | L; gbCPUTicks = 8; }; // SRL n
OPCB[0x3E] = function () { var M = MEMR(HL); FC = M & 1; M = M >> 1; FN = 0; FH = 0; FZ = M == 0 ? 1 : 0; MEMW(HL, M); gbCPUTicks = 16; }; // SRL n
OPCB[0x3F] = function () { FC = RA & 1; RA = RA >> 1; FN = 0; FH = 0; FZ = RA == 0 ? 1 : 0; gbCPUTicks = 8; }; // SRL n
...
MN[0xEA] = function () { return 'LD (0x' + hex4((MEMR(PC + 2) << 8) | MEMR(PC + 1)) + '),A'; };
